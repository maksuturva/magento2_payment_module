...
class Data extends \Magento\Framework\App\Helper\AbstractHelper implements \Svea\Maksuturva\Api\MaksuturvaHelperInterface
{
    ...
    public function statusQuery(\Magento\Sales\Model\Order $order){
        ...
        try {
            ...
            $this->sveaLoggerInfo("Status for order " . $order->getIncrementId() . " is " . $result['message']);
        } catch (\Exception $e) {
            $this->sveaLoggerError("Status for order " . $order->getIncrementId() . " failed, message " . $e->getMessage());
        }
    }
